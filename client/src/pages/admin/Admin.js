                  ...
                  stats.recentOrders.map(o => (
                    <div key={o._id} className="recent-order">
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <span className={`badge badge-${o.status === 'Delivered' ? 'success' : o.status === 'Cancelled' ? 'danger' : 'primary'}`}>
                          {o.status}
                        </span>
                      </div>
                    </div>
                  ...
                  <tbody>
                    {orders.map(o => (
                      <tr key={o._id}>
                        ...
                        <td>
                          <p className="fw-500">{o.user?.name || '—'}</p>
                          <p className="fs-12 text-muted">{o.user?.email || '—'}</p>
                        </td>
                        <td>{o.orderItems?.length || 0} item(s)</td>
                        <td className="fw-500">{fmt(o.totalPrice)}</td>
                        <td>
                          {o.paymentMethod}<br/>
                          <span className={`badge badge-${o.isPaid ? 'success' : 'warning'}`}>
          ...
          <div>
            ...
            {products.length === 0 ? (
              <div className="empty-state">
                ...
                <Link to="/admin/product/new" className="btn btn-primary" style={{ marginTop: 16 }}>Add First Product</Link>
              </div>
            ) : (
              <div className="admin-table-wrap">
                <table className="admin-table">
                  <thead>
                    <tr>
                      ...
                      <th>MRP</th><th>Stock</th><th>Sales</th><th>Rating</th><th>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {products.map(p => (
                      <tr key={p._id}>
                        ...
                        <td>
                          <span style={{ color: p.stock === 0 ? '#e91e8c' : p.stock <= 5 ? '#ff9800' : '#2ecc71' }}>
                  ...
                  <tbody>
                    {users.map(u => (
                      <tr key={u._id}>
                        ...
                        <td>{u.phone || '—'}</td>
                        <td>
                          <span className={`badge badge-${u.isAdmin ? 'danger' : 'muted'}`}>
